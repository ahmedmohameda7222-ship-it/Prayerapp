...
function announcementDb(parsed: ReturnType<typeof parseAnnouncement>) {
  return {
    ...
    display_style: parsed.displayStyle,
    display_from: parsed.displayFrom,
    display_until: parsed.displayUntil,
  };
}
...
export async function createAnnouncementAction(token: string, data: Record<string, string>) {
  return runAuditedAction(token, {
    ...
    const publishabilityError = validateAnnouncementPublishability(parsed);
    if (publishabilityError) return { success: false, error: publishabilityError };
  ...
  return runAuditedAction(token, { action: "announcement.publish", entityType: "announcement", entityId, metadata: { published: nextPublished } }, async () => {
    ...
    if (nextPublished) {
      const { data: row, error: readError } = await client
        .from("announcements")
        .select("id,title,title_ar,title_en,title_de,title_tr,message_ar,message_de,is_urgent,published")
        .eq("id", entityId)
        .maybeSingle();
      ...
      const validationError = validateDisplayPublishableContent("announcement", {
        ...
        messageDe: row.message_de || undefined,
      })[0];
