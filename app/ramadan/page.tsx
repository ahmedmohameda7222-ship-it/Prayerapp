  ...
  return (
    <AppShell>
      ...
      {data && day ? <div className="grid gap-5">
        ...
        <div className="grid grid-cols-2 gap-3">{[[t("ramadan.imsak"), day.imsak], [t("ramadan.fajr"), day.fajr], [t("ramadan.iftarMaghrib"), day.iftar], [t("ramadan.taraweeh"), day.taraweeh]].map(([label, value]) => <Card key={label}><p className="text-xs font-bold uppercase text-[var(--color-muted)]">{label}</p><p className="mt-1 text-2xl font-extrabold text-[var(--color-emerald)]"><FormattedTime time={value as string} /></p></Card>)}</div>
        <section><SectionTitle>{t("ramadan.announcements")}</SectionTitle><div className="grid gap-3">{data.announcements.length ? data.announcements.map((item) => <AnnouncementCard key={item.id} announcement={item} />) : <EmptyState message={t("ramadan.noAnnouncements")} />}</div></section>
