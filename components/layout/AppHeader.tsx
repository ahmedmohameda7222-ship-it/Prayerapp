  ...
  return (
    <header className="mb-5 grid grid-cols-[48px_1fr_48px] items-center gap-3">
      <div className="grid h-12 w-12 place-items-center rounded-full bg-[var(--color-emerald)] shadow-[var(--shadow-card)]">
        <Image src="/assets/app-icon-main.png" alt="Deggendorf Prayer app icon" width={34} height={34} className="rounded-full" priority />
      </div>
      <h1 className="font-brand text-center text-[22px] font-semibold text-[var(--color-emerald)]">{title}</h1>
      <button aria-label="Notifications" className="grid h-12 w-12 place-items-center rounded-full border border-[var(--color-border)] bg-[var(--color-card)] text-[var(--color-emerald)] shadow-[var(--shadow-soft)]">
