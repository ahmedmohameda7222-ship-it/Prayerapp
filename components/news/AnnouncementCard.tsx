    ...
    <article className={`card flex gap-3 p-4 ${announcement.isUrgent ? "border-l-4 border-l-[var(--color-gold)] bg-[#fff9e8]" : ""}`}>
      ...
      <div className="min-w-0 flex-1">
        <div className="mb-1 flex items-center gap-2">
          <h3 className="font-bold text-[var(--color-charcoal)]">{announcement.title}</h3>
          {announcement.isUrgent ? <Badge tone="gold">Urgent</Badge> : null}
        </div>
        <p className="line-clamp-2 text-sm leading-6 text-[var(--color-muted)]">{announcement.message}</p>
        <p className="mt-2 text-xs font-bold text-[var(--color-gold-dark)]">{announcement.type} · {announcement.createdAt.slice(0, 10)}</p>
      </div>
