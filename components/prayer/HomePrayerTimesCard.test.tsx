...
vi.mock("@/components/providers/AuthProvider", () => ({ usePublicAuth: () => ({ user: null }) }));
vi.mock("@/components/providers/AppPreferencesProvider", () => ({ useAppPreferences: () => ({ pushStatus: "disabled", enableNotifications: vi.fn() }) }));
vi.mock("@/components/providers/NativeAndroidProvider", () => ({ useNativeAndroid: () => ({ isNative: false, requestPermissions: vi.fn() }) }));
vi.mock("@/components/providers/AdhanAudioProvider", () => ({
  useAdhanAudio: () => ({ playbackStatus: "idle", activeSoundId: null, setPrayerSound: vi.fn(), previewSound: vi.fn(), stopAudio: vi.fn() }),
}));
vi.mock("@/components/providers/TimeFormatProvider", () => ({ useTimeFormat: () => ({ timeFormat: "24-hour" }) }));
vi.mock("@/lib/supabase/client", () => ({ createClient: () => null }));
vi.mock("@/lib/i18n/use-translation", () => ({
  useTranslation: () => ({
    locale: "en",
    t: (key: string) => ({
      "prayer.fajr": "Fajr", "prayer.sunrise": "Sunrise", "prayer.dhuhr": "Dhuhr", "prayer.asr": "Asr", "prayer.maghrib": "Maghrib", "prayer.isha": "Isha",
      "prayer.prayer": "Prayer", "prayer.azan": "Adhan", "prayer.iqama": "Iqama", "prayer.todaysPrayerTimes": "Today's Prayer Times", "prayer.khatira": "Khatira", "prayer.minutes": "min",
      "phase1.reminderOn": "Reminder on", "phase1.reminderOff": "Reminder off", "phase1.reminderSaveError": "Save error", "phase1.combinedIsha": "Combined Salat Isha",
    }[key] || key),
  }),
}));
...
const iqamaTimes: PrayerIqamaTimes = { fajr: "04:15", dhuhr: "13:30", asr: "17:30", maghrib: "20:45", isha: "23:10" };

describe("HomePrayerTimesCard", () => {
  it("keeps normal derived Isha on the Isha row and combined Isha inside the Maghrib program", () => {
    const { container } = render(<HomePrayerTimesCard prayer={prayer} iqamaTimes={iqamaTimes} activePrayer="isha" />);
  ...
  it("keeps six prayer rows and reminder bells only for the five obligatory prayers", () => {
    const { container } = render(<HomePrayerTimesCard prayer={prayer} iqamaTimes={iqamaTimes} activePrayer="asr" />);
