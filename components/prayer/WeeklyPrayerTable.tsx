...
export function WeeklyPrayerTable({
  ...
  const iqamaFor = (day: PrayerTime, name: PrayerName) =>
    name === "sunrise" || (name === "dhuhr" && isFridayIso(day.date))
      ? undefined
      : iqamaByDate[day.date]?.[name];
      ...
      <div className="mobile-prayer-days">
        {times.map((day) => (
          <article key={day.id} className="mobile-prayer-day" data-today={day.date === selectedDate ? "true" : undefined}>
            <header className="mobile-prayer-day-header"><time dateTime={day.date}>{formatShortDate(day.date, locale)}</time></header>
            <div className="mobile-prayer-day-grid">
              {prayerOrder.map((name) => {
                const iqama = iqamaFor(day, name);
      ...
      <div className="desktop-prayer-table overflow-x-auto rounded-[18px] border border-[var(--color-border)] bg-[var(--color-card)]">
        <table className="w-full min-w-[680px] border-collapse text-left text-sm">
          <thead className="bg-[var(--color-emerald)] text-[var(--color-card)]">
            <tr><th className="px-4 py-3">{t("times.day")}</th>{prayerOrder.map((name) => <th key={name} className="px-3 py-3">{columnLabel(name)}</th>)}</tr>
          </thead>
          <tbody>
            {times.map((day) => (
