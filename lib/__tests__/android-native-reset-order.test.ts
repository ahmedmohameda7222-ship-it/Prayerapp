...
describe("Android native reset ordering", () => {
  it("invalidates the account generation and queues authority revocation before cancelling alarms while preserving alarm metadata until cancellation", () => {
    ...
    const reset = bridge.indexOf("store.resetAccountStateAndQueueAuthorityRevocation()");
    ...
    const resetMethod = store.slice(
      store.indexOf("public int resetAccountStateAndQueueAuthorityRevocation()"),
      store.indexOf("public void clearAccountState()"),
    );
    expect(resetMethod).toContain("PENDING_AUTHORITY_REVOCATION");
