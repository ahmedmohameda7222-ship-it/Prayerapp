...
describe("Android direct-APK update system", () => {
  ...
  it("uses exact-tag authority resolution for the same-domain APK endpoint", () => {
    ...
    const releaseServer = source("lib/android-release-server.ts");
    ...
    expect(verifiedDelivery).toContain("getExpectedAndroidRelease");
    expect(verifiedDelivery).not.toContain("getLatestAndroidRelease({ fresh: true })");
    ...
    expect(releaseServer).toContain("/releases/tags/");
    expect(releaseServer).toContain("AUTHORITY_REVALIDATE_SECONDS");
    expect(releaseServer).toContain("signal");
