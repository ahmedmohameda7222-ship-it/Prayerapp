...
describe("Phase 1 account and personalization contracts", () => {
  ...
  it("targets only the five canonical prayers with optional pre-Adhan and mandatory Adhan notifications", () => {
    ...
    expect(delivery).toContain("export const prayerNames = {");
    expect(delivery).toContain("fajr:");
    expect(delivery).toContain("dhuhr:");
    expect(delivery).toContain("asr:");
    expect(delivery).toContain("maghrib:");
    expect(delivery).toContain("isha:");
    expect(delivery).not.toContain("sunrise:");
  ...
  it("retires the old global timing selector and fake countdown placeholder", () => {
    ...
    expect(countdown).toContain("stateFor(effectiveSchedule, iqamaByDate, new Date(initialNow), timezone)");
  });
